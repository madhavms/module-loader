...
const WidgetSelector = () => {
  ...
  const handleWidgetSelect = async (selectedWidget) => {
    ...
    const renderFn = await loadRemoteModule(
      rendererConfig.url,
      rendererConfig.scope,
      rendererConfig.module
    );
    const widgetModule = loadRemoteModule(url, scope, widget);
    renderFn.default(widgetModule, widgetRoot.current);
  };
